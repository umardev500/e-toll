...
import { useClickOutside, useCloseModal, useUpdateStatusProduct } from '../../../hooks'
import { type ProductStatus, type ProductStatusRequest } from '../../../types'

interface Props {
    setState: React.Dispatch<React.SetStateAction<boolean>>
    id: number
    defaultStatus: ProductStatus
    setStatusCallback: (status: ProductStatus) => void
}

export const ProductStatusModal: React.FC<Props> = ({ setState, id, setStatusCallback, defaultStatus }) => {
    const [status, setStatus] = useState<ProductStatus>(defaultStatus)
    const overlayRef = useRef<HTMLDivElement>(null)
    const innerRef = useRef<HTMLDivElement>(null)

    const handleClick = (status: ProductStatus) => {
        setStatus(status)
    }
    ...
    const updateProduct = useUpdateStatusProduct()
    const handleSubmit = () => {
        console.log('submit')
        if (status === undefined) {
        ...
        const request: ProductStatusRequest = { status }
                ...
                <div ref={innerRef} style={{ width: 325 }} className="modal-inner bg-white rounded-lg">
                    ...
                    <div className="px-6 pb-5 pt-4">
                        ...
                        <div
                            ...
                            className={`${
                                status === 'sold' ? 'bg-blue-50 ring-2' : 'bg-gray-50 hover:bg-gray-100'
                            } cursor-pointer text-left mb-2.5 rounded-md py-3 px-4 roboto text-gray-500 font-medium`}
                        >
                            <span>Status Soldout</span>
                        </div>
                        <div
                            onClick={() => {
                                handleClick('deleted')
